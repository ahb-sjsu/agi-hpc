...
\begin{figure}[t]
  ...
  generate $V_4$; the quarter-turns are posited, not measured. The $V_4$ claim is
...
both scenario and surface form). This is a shakedown result; replication on a
second model and a second transform family is running. Two calibration caveats
stand: probe mid-layers, because the final-layer representation map fails
held-out validation, and require held-out validation of the map itself. The
lens remains early: a promising extension with its first measurement, not a
finished detector.
