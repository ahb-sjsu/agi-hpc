...
Find the bounding box of all non-zero cells in the input grid. Compute the center point of this bounding box (the midpoint between min/max row and min/max column). For each non-zero cell, reflect its value across both the horizontal and vertical axes passing through that center point.

This fills in all four symmetric positions for each cell:
1. The original cell (already present)
2. Its horizontal mirror (reflected across the horizontal axis)
3. Its vertical mirror (reflected across the vertical axis)
4. Its diagonal mirror (reflected across both axes)
...
def transform(grid):
    ...
    # Start with a copy of input (preserves existing non-zero cells)
    out = g.copy()
    
    # For each non-zero cell, reflect across both symmetry axes
    for r, c in zip(rows, cols):
        val = g[r, c]
        
        # Horizontal reflection (across center_r)
        r_h = int(2 * center_r - r)
        
        # Vertical reflection (across center_c)
        c_v = int(2 * center_c - c)
        
        # Place value at all four symmetric positions
        out[r_h, c] = val      # horizontal mirror
        out[r, c_v] = val      # vertical mirror
        out[r_h, c_v] = val    # diagonal mirror (both axes)
