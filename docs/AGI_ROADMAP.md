...
| **Chat ego as vMOE ensemble** | Planned — architecture pivoted 2026-04-20 | Ego is a **vMOE call per turn** that fans out across NRP frontier models + small locally-fine-tuned Atlas specialists (ErisML voice, memory-RAG voice, pattern voice). Frontier capability from the Envoy Gateway (qwen3, kimi, glm-4.7, minimax-m2, gpt-oss, olmo), identity from GV100-hosted QLoRA specialists. See README §"Cognitive ensemble (vMOE)". |
| **Self-hosted ego pod on NRP** | ~~Blocked~~ **Deprecated 2026-04-20** | NRP's Jobs-based policy doesn't support permanent serving, and "permanent higher brain in NRP" trades rental cost + policy friction for no capability win over managed-API-as-experts. PVC `erebus-ego-models` (300 Gi, Bound) is kept as artifact storage for fine-tune outputs; manifests in `deploy/k8s/erebus-ego/` are reserved as Plan B. |
| **Atlas llama.cpp fallback ego** | ~~Pending~~ **Superseded 2026-04-20** | Replaced by the vMOE ensemble. NRP-outage resilience now comes from local specialists continuing to serve + cascade to `gemma` 31B and `gemma-small` 8B as cheap/fast frontier voices. |
| **Local fine-tuned specialists (ErisML, memory-RAG, pattern)** | Planned | QLoRA-trained on Atlas GV100s during dreaming windows. 7B class each, Q8. Produces the "identity" voices the managed API can't be fine-tuned into. Adapter artifacts persisted to CephFS PVC as a side effect. |
| **LoRA hot-swap for dreaming adapters** | Active path — now the PRIMARY use of the fine-tune pipeline | No longer "deferred" — local specialist adapters ARE the dreaming pipeline's raison d'être. Produces adapters that load into the vMOE local experts. |
