...
\begin{itemize}
  \item \small LLMs increasingly gate \hi{safety-critical decisions} --- content moderation, clinical triage, legal analysis.
  \item \small A secure evaluator should judge the \hi{facts}, not their \hi{presentation}.
  \item \small \alert{Behavioral manipulation vulnerability}: the attacker changes morally \emph{irrelevant} surface features --- framing, tone, sensory detail --- never the underlying situation.
\end{itemize}
\end{column}
\begin{column}{0.42\textwidth}
\centering
\begin{tikzpicture}[>=Stealth, node distance=2.2mm,
  fact/.style={draw=ink, thick, rounded corners=1.5pt, fill=black!4,
    font=\scriptsize, align=center, inner sep=3pt, text width=30mm},
  word/.style={draw=cbgray, semithick, rounded corners=1.5pt, fill=white,
    font=\scriptsize, align=center, inner sep=2.5pt, text width=25mm},
  verd/.style={font=\scriptsize\bfseries, inner sep=2.5pt, rounded
    corners=1.5pt},
  arr/.style={->, semithick, draw=cbgray}]
\node[fact] (f) {the same facts};
\node[word, below left=4mm and -9mm of f] (d) {stated plainly};
\node[word, below right=4mm and -9mm of f] (e) {euphemistic\\rewording};
\node[verd, draw=cbblue, text=cbblue, below=3.5mm of d] (v1) {FLAG};
\node[verd, draw=cborange, text=cborange, below=3.5mm of e] (v2) {PASS};
\draw[arr] (f) -- (d);
\draw[arr] (f) -- (e);
\draw[arr] (d) -- node[left, font=\tiny, text=cbgray] {judge} (v1);
\draw[arr] (e) -- node[right, font=\tiny, text=cbgray] {judge} (v2);
\node[below=1.5mm of $(v1)!0.5!(v2)$, font=\tiny, text=cbgray,
  align=center] {same situation --- different verdict:\\the wording is an attack surface};
\end{tikzpicture}
\end{column}
\end{columns}
\vfill
...
\begin{itemize}
  ...
  \item Claude: \gold{zero observed sycophancy} (0/9 --- small $n$, directionally clear) --- but \gold{worst} emotional-anchoring recovery (20\%) and worst divided attention.
...
\begin{itemize}
  \item \small Euphemistic rewriting (morally invariant) lowers panel-averaged harm by \gold{$-14.0$ points} on the 0--70 scale (4/6 gold $>10$ pts); dramatic only $+7.3$.
  \item \small At a median-calibrated threshold, \gold{3 of 6} gold items silently reclassify \textsc{flag}$\rightarrow$\textsc{pass} {\small (existence proof: $n=6$ hand-audited gold items)}.
  \item \small \alert{Asymmetry}: far easier to \emph{evade} (hide harm) than to \emph{trigger} a false positive --- the dangerous direction.
\end{itemize}
\vfill
\centering
\begin{tikzpicture}[>=Stealth]
\draw[->, thick, cbgray!70] (0,0) -- (11.4,0);
\node[font=\scriptsize, text=cbgray] at (2.0,-0.35) {total harm (0--70) $\rightarrow$};
\draw[dashed, thick, ink] (5.6,-0.3) -- (5.6,1.55)
  node[above, font=\scriptsize] {threshold $T$};
\node[font=\tiny, text=cbgray] at (2.8,1.45) {\textsc{pass} side};
\node[font=\tiny, text=cbgray] at (8.6,1.45) {\textsc{flag} side};
% six gold items: base (blue) on the flag side; euphemistic rewrite (orange)
% pulls each left; three cross T (schematic geometry; measured stats below)
\foreach \xb/\xe/\y in {6.3/4.1/0.45, 7.2/4.9/0.80, 8.4/5.2/1.15,
                        7.9/6.1/0.45, 9.3/7.4/0.80, 10.4/8.6/1.15}{
  \fill[cbblue] (\xb,\y) circle (2.1pt);
  \fill[cborange] (\xe,\y) circle (2.1pt);
  \draw[->, semithick, cborange!80] (\xb-0.12,\y) -- (\xe+0.14,\y);
}
\node[font=\tiny, text=cbgray, align=left, anchor=west] at (0.15,0.85)
  {\textcolor{cbblue}{$\bullet$} original\\[-2pt]
   \textcolor{cborange}{$\bullet$} euphemistic};
\node[font=\tiny, text=cbgray, anchor=east] at (11.3,-0.62)
  {schematic geometry --- the measured statistics are the bullets above};
\end{tikzpicture}
\end{frame}
...
\begin{tikzpicture}[>=Stealth, node distance=5mm,
  ...
  arr/.style={->, semithick, draw=cbgray}]
\node[stage] (t) {scenario\\text};
\node[stage, right=of t] (fx) {LLM extracts\\\textbf{ethical facts}};
\node[stage, right=of fx, fill=black!4, draw=cbgray]
  (rl) {\texttt{GenevaEMV3}\\fixed rule module};
\node[stage, right=of rl] (v) {typed verdict\\{\tiny \textsc{forbid} $\cdots$ \textsc{strongly\_prefer}}};
\draw[arr] (t) -- (fx);
\draw[arr] (fx) -- (rl);
\draw[arr] (rl) -- (v);
\draw[->, very thick, cborange]
  ($(fx.north)+(0,7mm)$) -- (fx.north)
  node[midway, right=1mm, font=\scriptsize, text=cborange,
    align=left] {euphemistic rewrite\\lands \textbf{here}};
\node[below=1.5mm of rl, font=\tiny, text=cbgray] {rules unchanged};
\end{tikzpicture}
\end{center}
\vfill
\begin{block}{Across 161 scenario--model cases}
Euphemistic rewriting flips \textsc{forbid/avoid}$\rightarrow$permissive in \gold{13.7\%} of cases; \textsc{forbid} verdicts fall \gold{44 $\rightarrow$ 27} ($-39\%$).
\end{block}
...
{\scriptsize\\[2pt] DEME v3 is our own kernel, \hi{frozen} for this test (nothing tuned to these flips) --- a stress substrate, not the ground truth (which is the audited meaning-invariance of the rewrites).}
\end{frame}

% ---------------------------------------------------------------
\begin{frame}{Since submission --- a measured defense}
{\small\color{cbgray} Second instantiation from here on: validated per-axis learned encoders (\texttt{xbse} --- one small trained scorer per axis) replace the LLM judges; same DEME kernel.\par}
\begin{center}
\begin{tikzpicture}[>=Stealth, node distance=4.5mm,
  stage/.style={draw=cbblue, thick, rounded corners=1.5pt, fill=cbblue!6,
    font=\scriptsize, align=center, inner sep=3pt},
  arr/.style={->, semithick, draw=cbgray}]
\node[stage] (x) {input $x$};
\node[stage, right=of x, draw=cborange, dashed, fill=cborange!6]
  (cls) {paraphrase class\\$x_1 \cdots x_m$ {\tiny(generated)}};
\node[stage, right=of cls] (enc) {validated\\encoders};
\node[stage, right=of enc, draw=cbteal, fill=cbteal!8]
  (avg) {\textbf{average}\\over the class};
\node[stage, right=of avg] (dec) {decide};
\draw[arr] (x) -- (cls);
\draw[arr] (cls) -- (enc);
\draw[arr] (enc) -- (avg);
\draw[arr] (avg) -- (dec);
\node[below=1.2mm of cls, font=\tiny, text=cborange, align=center]
  {the measured hole: a euphemistic $x$\\yields a class that \emph{stays} euphemistic};
\node[below=1.2mm of dec, font=\tiny, text=cbgray, align=center]
  {refusal $\rightarrow$ singleton\\$\rightarrow$ \textbf{escalate}};
\end{tikzpicture}
\end{center}
\begin{itemize}
  \item \small At scale (60 held-out items, $m=6$): raw drift \gold{0.407 $\rightarrow$ $\theta_d$ = 0.219} --- the mechanism \hi{halves} salience drift {\small ($\theta_d$: mean per-dimension decision movement)}. The registered bar $\theta_d \le 0.5$ \hi{binds for adversarial-register inputs} (raw 0.67--0.85); the defended run there is the registered open item.
  \item \small LLM paraphrasers \alert{refuse 24\%} of harmful inputs. A non-refusing red-team paraphraser (NLLB back-translation, 6 pivots): \gold{$\theta_d = 0.301$} on harmful content --- the weakness is the \hi{generator's}, not the mechanism's.
\end{itemize}
\vfill
...
{\small\textbf{ICC 0.969 = reliably \emph{measurable}. Are the dimensions \emph{distinct}?} A registered test on the \hi{fuller DEME 9-axis set $+$ \texttt{identity\_attack}} {\scriptsize(the moral-foundations superset the 7-D harm space of slides 2--3 projects from --- backup B1)}: a \hi{general moral-valence channel $G$} gate-passes (AUROC \gold{0.856}); five named axes are $\ge$\gold{0.98} predictable from $G$ alone.\par}
\smallskip
\begin{columns}[T]
...
\begin{block}{\small Why this doesn't undercut Findings 1--2}
\small Harm \emph{scores} collapse to $G$; \hi{vulnerability does not}. The attack surfaces that dissociate (Finding~2) are the specific axes that survive $G$ --- ``$G$ plus physical'' discards where manipulation lives.
\end{block}
\smallskip
{\footnotesize The \hi{judge panel's own 31$\times$7 matrix replicates it}: PC1 = \gold{54\%}, sole factor surviving parallel analysis; per-judge FAs agree.}
\end{column}
\end{columns}
\vfill
...
\begin{itemize}
  ...
  \item \hi{Prompt-level defenses are bounded}: explicit warnings recover only $\sim$38\% --- a ceiling co-occurring with universal overconfidence (ECE, calibration error, 0.19--0.42).
...
% ---------------------------------------------------------------
\begin{frame}{What we have \emph{not} shown --- limitations}
\begin{itemize}
  \item \small \hi{Adversarial defense is unproven at scale.} Class averaging halves \emph{natural}-paraphrase drift; against adversarial register the gold set ($n{=}6$) shows it \gold{does not hold} (3/3 still flip). Escalate-by-default carries these until a register-crossing generator exists; the at-scale adversarial run is registered, unrun.
  \item \small \hi{Small samples on two claims}: the end-to-end exploit is an $n{=}6$ existence proof; ``zero sycophancy'' is 0/9. Directionally clear, not population rates.
  \item \small \hi{Two instantiations.} Attack results are on LLM judges; defense/bifactor on the learned encoders. The native re-run of all five tracks through one pipeline is designed, not yet run.
  \item \small \hi{Corpus scope.} Moderation/social-norm corpora, predominantly English; the contraction's 0.863 is within-corpus-family. Cross-cultural and cross-family transfer untested.
\end{itemize}
\vfill
{\scriptsize Every number in the talk is hedged at its point of use; this slide collects the four that bound the claims.}
\end{frame}
