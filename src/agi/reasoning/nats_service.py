...
async def _respond_rpc(msg) -> None:
    ...
    try:
        ...
        logger.exception(
            "[ego-nats] bad rpc request on %s", getattr(msg, "subject", "?")
        )
        return
    logger.info("[ego-nats] deliberate(rpc): %s", str(event.payload)[:120])
    result = await asyncio.to_thread(_deliberate, event.payload)
    reply = Event.create(
        "ego", "agi.ego.deliberation", {**result, "request_id": event.id}
    )
...
async def _wire(fabric: NatsEventFabric) -> None:
    ...
    logger.info(
        "[ego-nats] deliberation online (ego=%s, council=%s); rpc=_rpc.agi.ego.deliberate",
        EGO_MODEL,
        COUNCIL_MODEL,
    )
...
async def _selftest() -> None:
    ...
    ev = Event.create(
        "selftest",
        "agi.ego.deliberate",
        {"prompt": "In one sentence: is the sky blue, and why?", "max_tokens": 150},
    )
