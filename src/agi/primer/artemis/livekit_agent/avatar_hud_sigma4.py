...
log = logging.getLogger("sigma4.avatar")
...
async def main() -> int:
    ...
    #       agi.rh.sigma4.say.direct, ARTEMIS speaks it).
...
async def _direct_say_bridge(running: asyncio.Event) -> None:
    ...
    Publish ``{"text": "..."}`` to ``agi.rh.sigma4.say.direct`` and
    ...
    try:
        ...
        return
    log.info("direct-say bridge online: agi.rh.sigma4.say.direct")
    ...
    try:
        await nc.subscribe("agi.rh.sigma4.say.direct", cb=_on_msg)
        while not running.is_set():
...
async def _chat_bridge(room: rtc.Room, running: asyncio.Event) -> None:
    ...
    Browser → NATS:
      ...
      ``agi.rh.sigma4.chat.in.<from_id>`` so the chat service picks
      it up, persists, and routes through Primer.

    NATS → Browser:
      Subscribe to ``agi.rh.sigma4.chat.out.*`` and
      ``agi.rh.sigma4.chat.broadcast``; forward each as
    ...
    def _on_data(data, participant, *_rest) -> None:
        ...
        subj = f"agi.rh.sigma4.chat.in.{from_id}"
        payload = json.dumps(chat_msg, separators=(",", ":")).encode()
        asyncio.create_task(nc.publish(subj, payload))

    try:
        await nc.subscribe("agi.rh.sigma4.chat.out.*", cb=_on_out)
        await nc.subscribe("agi.rh.sigma4.chat.broadcast", cb=_on_out)
        while not running.is_set():
            await asyncio.sleep(1)
    finally:
        try:
...
async def _sheets_bridge(room: rtc.Room, running: asyncio.Event) -> None:
    """Subscribe to agi.rh.sigma4.sheet.* and forward to DataChannel.
    ...
    try:
        await nc.subscribe("agi.rh.sigma4.sheet.>", cb=_on_msg)
